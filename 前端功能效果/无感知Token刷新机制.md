...
**前端用短期 Access Token（存 localStorage）请求接口，长期 Refresh Token 存在 HTTP-only Cookie，前端无法直接读取,防止xss跨站脚本攻击，但通过 withCredentials: true 发请求时浏览器会自动携带。Access Token 过期时，调用刷新接口获取新 Token，多请求同时刷新会排队保证只刷新一次，刷新失败则清空 Token 并跳登录页，既安全又支持并发处理**
