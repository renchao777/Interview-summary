...
**前端用 短期 Access Token 存在 localStorage 调接口,长期 Refresh Token 放在Cookie中设置 HttpOnly 和 SameSite 属性（SameSite=Lax/Strict），防 XSS 与csrf跨站请求. 请求时加 withCredentials: true(跨域请求可自动携带cookie), 再在请求头带随机生成的 CSRF Token. 当 Access Token 过期时, 通过刷新接口获取新 Token，多请求同时刷新会排队保证只刷新一次，刷新失败则清空 Token 并跳登录页**
